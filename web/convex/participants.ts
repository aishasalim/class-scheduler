...
const PARTICIPANTS = [
  ...
  { id: "rishika-sikka", name: "Rishika Sikka", major: "Computer Science", gender: "F", birthday: "February 24", phone: "5039280246" },
...
export const seed = mutation({
  args: {},
  handler: async (ctx) => {
    const existing = await ctx.db.query("participants").first();
    if (existing) return { seeded: false, count: 0 };
    for (const p of PARTICIPANTS) {
      await ctx.db.insert("participants", p);
    }
    return { seeded: true, count: PARTICIPANTS.length };
  },
});

/** Find participant by full name (e.g. "Aisha Salimgereyeva") for login. */
export const findByName = query({
  args: { name: v.string() },
  handler: async (ctx, { name }) => {
    const normalized = name.trim().toLowerCase();
    const all = (await ctx.db.query("participants").collect()) as { _id: unknown; name: string; id: string }[];
    return all.find((p) => p.name.toLowerCase() === normalized)
      ?? all.find((p) => p.name.toLowerCase().includes(normalized) || normalized.includes(p.name.toLowerCase()));
  },
});

/** Get participant by string id (e.g. "aisha-salimgereyeva") for resolving to Convex _id. */
export const getByStringId = query({
  args: { stringId: v.string() },
  handler: async (ctx, { stringId }) => {
    const all = (await ctx.db.query("participants").collect()) as { _id: unknown; id: string }[];
    return all.find((p) => p.id.toLowerCase() === stringId.toLowerCase()) ?? null;
  },
});
